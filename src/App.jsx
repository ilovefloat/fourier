...
export default function App() {
  ...
  // --- FIX 1: Reset index when mode changes ---
  useEffect(() => {
  ...
  const initMode = useCallback((m, lvl) => {
    ...
    // --- FIX 2: Ensure index resets during initialization ---
    setActiveTermIndex(0);
  ...
  // --- FIX 3: Safe fallback for activeKey ---
    ...
    <div className="fixed inset-0 bg-slate-950 text-slate-200 flex flex-col font-sans overflow-hidden">
      <header className="flex-none flex justify-between items-center px-3 py-2 bg-slate-900 border-b border-slate-800 safe-top z-10">
        ...
        <div className="flex gap-2 items-center">
          ...
          <button onClick={() => initMode(mode, activeLevel)} className="p-1.5 hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-white active:scale-95" title="Restart Level">
      ...
      <div className="flex-none w-full flex justify-between items-center px-3 py-1.5 bg-slate-950 border-b border-slate-900 shadow-sm z-10">
        ...
        <div className="flex items-center gap-2 bg-slate-900 px-2 py-1 rounded-lg border border-slate-800 shadow-inner">
          ...
          <div className={`text-sm md:text-base font-mono font-black w-10 text-right ${getErrorColor(errorNorm)} transition-colors duration-300`}>{errorNorm.toFixed(2)}</div>
        </div>
      </div>
          ...
          <svg viewBox={`0 0 ${GRAPH_SIZE} ${GRAPH_SIZE}`} className="absolute inset-0 w-full h-full" preserveAspectRatio="xMidYMid meet">
            {mode === 'complex' ? (
              ...
              <>
                ...
                {[...Array(5)].map((_, i) => (<line key={i} x1={PADDING} y1={PADDING + (i / 4) * (GRAPH_SIZE - 2*PADDING)} x2={GRAPH_SIZE-PADDING} y2={PADDING + (i / 4) * (GRAPH_SIZE - 2*PADDING)} stroke="#1e293b" strokeWidth="1" />))}
              </>
            )}
      ...
      <section className="flex-none bg-slate-900 border-t border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] flex flex-col safe-bottom relative z-10 pt-2 pb-4" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
        {showNext ? (
          <div className="flex flex-col items-center justify-center px-4 py-6 animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center gap-2 mb-2 text-emerald-400"><Trophy className="w-6 h-6" /><h2 className="text-xl md:text-2xl font-black tracking-wide uppercase">Perfect Match!</h2></div>
            <p className="text-slate-400 text-xs md:text-sm mb-4 text-center">Waveforms aligned. Error norm reached 0.00.</p>
            <button onClick={handleNextLevel} className={`w-full max-w-sm inline-flex items-center justify-center px-6 py-3 font-black rounded-xl transition-all text-base md:text-lg active:scale-95 touch-manipulation ${mode === 'complex' ? 'bg-gradient-to-r from-cyan-400 to-emerald-400 text-slate-950 shadow-[0_0_20px_-5px_rgba(45,212,191,0.5)]' : 'bg-gradient-to-r from-blue-400 to-cyan-400 text-slate-950 shadow-[0_0_20px_-5px_rgba(56,189,248,0.5)]'}`}>Start Level {activeLevel + 1} <ArrowRight className="ml-2 w-5 h-5" /></button>
          </div>
        ) : (
          <>
            <div className="flex justify-center gap-1 mb-2 px-2 overflow-hidden">
              {activeKeys.map((k, idx) => (<div key={k} className={`h-1.5 rounded-full transition-all duration-300 flex-shrink-0 ${idx === activeTermIndex ? 'w-5 bg-cyan-400' : 'w-1.5 bg-slate-700'}`} />))}
            </div>
...
<button onClick={nextTerm} disabled={activeTermIndex === activeKeys.length - 1} className="p-2 bg-slate-800 rounded-full text-slate-300 disabled:opacity-30 active:scale-95 transition-all touch-manipulation"><ChevronRight className="w-5 h-5" /></button>
              ...
              {mode === 'complex' ? (
                <>
                  <div className="flex justify-between items-center gap-2 px-2 mb-2 bg-slate-800/50 p-1.5 rounded-xl border border-slate-700/50">
                    ...
                    <button onClick={() => stepValue('r', -1)} disabled={activeCoeff.r <= -3} className="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 flex items-center justify-center bg-slate-800 rounded-xl text-rose-400 disabled:opacity-30 active:scale-90 border border-slate-700 touch-manipulation"><Minus className="w-5 h-5" strokeWidth={3} /></button>
                    <div className="flex-1 text-center font-mono"><div className="text-2xl md:text-3xl font-black text-white tracking-tight">{activeCoeff.r > 0 ? '+' : ''}{activeCoeff.r.toFixed(1) === '-0.0' ? '0.0' : activeCoeff.r.toFixed(1)}</div></div>
                    <button onClick={() => stepValue('r', 1)} disabled={activeCoeff.r >= 3} className="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 flex items-center justify-center bg-slate-800 rounded-xl text-emerald-400 disabled:opacity-30 active:scale-90 border border-slate-700 touch-manipulation"><Plus className="w-5 h-5" strokeWidth={3} /></button>
                  </div>
                  <div className="flex justify-between items-center gap-2 px-2 bg-slate-800/50 p-1.5 rounded-xl border border-slate-700/50">
                    ...
                    <button onClick={() => stepValue('i', -1)} disabled={activeCoeff.i <= -3} className="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 flex items-center justify-center bg-slate-800 rounded-xl text-rose-400 disabled:opacity-30 active:scale-90 border border-slate-700 touch-manipulation"><Minus className="w-5 h-5" strokeWidth={3} /></button>
                    <div className="flex-1 text-center font-mono"><div className="text-2xl md:text-3xl font-black text-white tracking-tight">{activeCoeff.i > 0 ? '+' : ''}{activeCoeff.i.toFixed(1) === '-0.0' ? '0.0' : activeCoeff.i.toFixed(1)}i</div></div>
                    <button onClick={() => stepValue('i', 1)} disabled={activeCoeff.i >= 3} className="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 flex items-center justify-center bg-slate-800 rounded-xl text-emerald-400 disabled:opacity-30 active:scale-90 border border-slate-700 touch-manipulation"><Plus className="w-5 h-5" strokeWidth={3} /></button>
                  </div>
                </>
              ) : (
                <div className="flex justify-between items-center gap-4 px-2 py-4">
                  <button onClick={() => stepValue('val', -1)} disabled={activeCoeff <= -3} className="w-16 h-16 md:w-20 md:h-20 flex-shrink-0 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded-2xl text-rose-400 disabled:opacity-30 active:scale-90 transition-all border border-slate-700 shadow-lg touch-manipulation"><Minus className="w-8 h-8 md:w-10 md:h-10" strokeWidth={3} /></button>
                  <div className="flex-1 text-center font-mono"><div className="text-4xl md:text-5xl font-black text-white tracking-tight">{activeCoeff > 0 ? '+' : ''}{activeCoeff.toFixed(1) === '-0.0' ? '0.0' : activeCoeff.toFixed(1)}</div></div>
                  <button onClick={() => stepValue('val', 1)} disabled={activeCoeff >= 3} className="w-16 h-16 md:w-20 md:h-20 flex-shrink-0 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded-2xl text-emerald-400 disabled:opacity-30 active:scale-90 transition-all border border-slate-700 shadow-lg touch-manipulation"><Plus className="w-8 h-8 md:w-10 md:h-10" strokeWidth={3} /></button>
                </div>
              )}
            </div>
          </>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-slate-900 border border-slate-700/50 rounded-3xl w-full max-w-md shadow-2xl flex flex-col max-h-[80vh] transform transition-all overflow-hidden animate-in zoom-in-95 duration-200 relative">
            <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-900/50">
              ...
              <button onClick={() => setIsLevelSelectorOpen(false)} className="p-2 text-slate-400 hover:text-white bg-slate-800 hover:bg-slate-700 rounded-full transition-colors active:scale-95"><X className="w-4 h-4" /></button>
            </div>
            <div className="overflow-y-auto custom-scrollbar p-5">
              <div className="grid grid-cols-4 gap-3">
                {Array.from({ length: Math.max(20, Math.ceil(maxUnlocked / 20) * 20) }, (_, i) => i + 1).map(l => {
                  ...
                  return (
                    <button key={l} disabled={!isUnlocked} onClick={() => { setLevels(prev => ({ ...prev, [mode]: l })); initMode(mode, l); setIsLevelSelectorOpen(false); }} className={`aspect-square rounded-xl font-black text-base flex items-center justify-center transition-all duration-300 relative ${isCurrent ? 'bg-gradient-to-br from-cyan-400 to-emerald-400 text-slate-950 shadow-[0_0_15px_-3px_rgba(45,212,191,0.6)] scale-110 z-10' : isUnlocked ? 'bg-slate-800 text-slate-300 border border-slate-700 hover:bg-slate-700 hover:text-white hover:border-slate-500 shadow-sm active:scale-95 z-0' : 'bg-slate-900/40 text-slate-700 border border-slate-800/60 cursor-not-allowed z-0'}`}>
      ...
      <style dangerouslySetInnerHTML={{__html: `.safe-top { padding-top: max(env(safe-area-inset-top), 16px); } .safe-bottom { padding-bottom: max(env(safe-area-inset-bottom), 16px); } .touch-manipulation { touch-action: manipulation; } .custom-scrollbar::-webkit-scrollbar { width: 4px; height: 4px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #334155; border-radius: 10px; } .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #475569; }`}} />
    </div>
  );
}
